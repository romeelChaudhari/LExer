 ...
 * with an indicatio3n of its location in the source program to be used for error
...
public class Lexer {
    ...
    public static void main(String args[]) {
        ...
        if(args.length == 0)
        {
            System.out.println("\nError: Line not found, enter valid command!");
            System.exit(0);
        }
        else {
            ...
            try {
                lex = new Lexer("test.x");
                while (true) {
                    tok = lex.nextToken();
                    System.out.printf("%-11s Left: %-8d Right: %-8d Line Number: %-3d %n", tok.toString(), tok.getLeftPosition(),tok.getRightPosition(), tok.getLineNum());
                }
            } catch (Exception e) {
